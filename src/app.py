...
page = st.sidebar.selectbox(
    "Choose a page:",
    ["🏠 Home", "📊 Data Explorer", "🔮 Predictor", "📈 Analytics", "🗺️ Geographic View", "📋 About"]
)
...
# Home page
if page == "🏠 Home":
    st.markdown('<h1 class="main-title">♻️ Waste Management Analytics & Predictor</h1>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        - 🗺️ **Geographic Visualization**: Interactive maps and regional analysis
        - 📈 **Trend Analysis**: Historical patterns and future projections
        """)
    
    with col2:
        st.markdown("""
        - 🏙️ **Multi-City Support**: Mumbai, Delhi, Bengaluru, Kolkata, Chennai
        - ♻️ **Waste Type Analysis**: Plastic, Organic, E-Waste, Construction, Hazardous
        - 🏛️ **Municipal Insights**: Efficiency scoring and cost analysis
...
# Predictor page
    ...
    if model is not None:
        ...
        with st.form("prediction_form"):
            ...
            with col1:
                city = st.selectbox("🏙️ City/District", city_options)
                waste_type = st.selectbox("🗑️ Waste Type", waste_type_options)
            ...
            with col2:
                municipal_efficiency = st.slider("🏛️ Municipal Efficiency Score (1-10)", 1, 10, 7)
        ...
        if submitted:
            try:
                ...
                <div class="prediction-result">
                    <h2>🔮 Predicted Recycling Rate</h2>
                ...
                with col1:
                    if status_color == "error":
                        ...
                        st.markdown("- 🏛️ Improve municipal efficiency")
                        ...
                        st.markdown("- 🏗️ Invest in better infrastructure")
                ...
                with col2:
                    ...
                    st.markdown(f"- 🗑️ **Waste Type:** {waste_type}")
                    st.markdown(f"- 🚛 **Disposal Method:** {disposal_method}")
                    st.markdown(f"- ♨️ **Waste Generated:** {waste_generated:.1f} tons/day")
                    st.markdown(f"- 👥 **Population Density:** {population_density:,} people/km²")
                    st.markdown(f"- 🏛️ **Municipal Efficiency:** {municipal_efficiency}/10")
                ...
                try:
                    ...
                    if predictions_path.exists():
                        ...
                        st.warning("⚠️ predictions.csv not found. Please create it manually with the required columns.")
                    ...
                    st.warning(f"⚠️ Could not save prediction: {save_error}")
...
# Analytics page
    ...
    if df is not None:
        ...
        with col1:
            st.markdown("### 🗑️ Waste Type Analysis")
            try:
        ...
        with col2:
            st.markdown("### 🏛️ Municipal Efficiency Impact")
            try:
...
# Geographic View page
    ...
    st.markdown('<h1 class="main-title">🗺️ Geographic Analysis</h1>', unsafe_allow_html=True)
    
    if df is not None:
        # City-wise performance map
        st.markdown("## 🏙️ City Performance Overview")
...
st.markdown(
    "<div style='text-align: center; color: #666; padding: 1rem;'>"
            "♻️ Waste Management Analytics & Predictor | Made By PrabalRoy7884 "
    "</div>",
    unsafe_allow_html=True
)